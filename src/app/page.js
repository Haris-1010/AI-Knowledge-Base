...
export default function WorkspacePage() {
  ...
  const fetchSources = useCallback(async (kbId) => {
    try {
  ...
  const fetchChats = useCallback(async (kbId) => {
    try {
  ...
  /* eslint-disable react-hooks/set-state-in-effect -- data fetching in useEffect is intentional */
  useEffect(() => {
    if (session?.user) {
      fetchKbs();
    } else {
  ...
  }, [selectedKb, fetchSources, fetchChats]);
                ...
                {messages.length === 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center text-center py-12 px-6 max-w-lg mx-auto my-auto bg-zinc-950/50 border border-zinc-850/40 backdrop-blur-md p-8 rounded-3xl shadow-2xl shadow-indigo-500/5 animate-slide-up">
                    ...
                    <p className="text-xs text-zinc-300 mt-4 max-w-sm italic leading-relaxed border-l-2 border-indigo-500/30 pl-4 py-1.5 text-left">
                      &ldquo;{selectedKb.description || "I am your custom RAG knowledge assistant. Ask me anything!"}&rdquo;
                    </p>
